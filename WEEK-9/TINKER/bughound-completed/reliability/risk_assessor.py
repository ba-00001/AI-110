...
def assess_risk(
    ...
    changed_line_delta = abs(len(fixed_lines) - len(original_lines))
    ...
    if original_lines and changed_line_delta >= max(3, int(len(original_lines) * 0.4)):
        score -= 25
        reasons.append("Fix changes a large portion of the file.")
    ...
    # Auto-fix policy
    # ----------------------------
    only_low_severity = bool(issues) and all(
        str(issue.get("severity", "")).lower() == "low" for issue in issues
    )
    should_autofix = level == "low"

    if only_low_severity and any("large portion" in reason.lower() for reason in reasons):
        should_autofix = False
        reasons.append("Low-severity issues should not trigger auto-fix when the edit footprint is large.")
