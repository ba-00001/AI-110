...
class WeakJsonClient:
    def complete(self, system_prompt: str, user_prompt: str) -> str:
        if "Return ONLY valid JSON" in system_prompt:
            return '[{"type": "Code Quality", "severity": "Low", "msg": ""}]'
        return "def f():\n    print('hi')\n"


def test_llm_analysis_falls_back_when_json_is_incomplete():
    agent = BugHoundAgent(client=WeakJsonClient())
    code = "def f():\n    print('hi')\n    return True\n"
    result = agent.run(code)

    assert any(issue.get("type") == "Code Quality" for issue in result["issues"])
    assert any("structurally weak or incomplete" in entry.get("message", "") for entry in result["logs"])
