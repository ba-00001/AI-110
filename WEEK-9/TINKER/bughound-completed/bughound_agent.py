...
class BugHoundAgent:
    ...
    def analyze(self, code_snippet: str) -> List[Dict[str, str]]:
        ...
        if not self._is_valid_issue_list(issues):
            self._log("ANALYZE", "LLM output was structurally weak or incomplete. Falling back to heuristics.")
            return self._heuristic_analyze(code_snippet)
    ...
    def _is_valid_issue_list(self, issues: List[Dict[str, str]]) -> bool:
        if not issues:
            return False
        ...
        for issue in issues:
            ...
            if issue_type and message and severity in allowed_severities:
                valid_count += 1
