"""
Playlist Chaos — Streamlit UI

This file is also intentionally commented like I would explain it to a teammate.
The activity is about debugging AI-written code, so readability matters.
"""
...
# -----------------------------
def init_state() -> None:
    """Initialize Streamlit session state once per browser session."""
    if "songs" not in st.session_state:
        st.session_state.songs = default_songs()
    if "profile" not in st.session_state:
        # Make a copy so I don't mutate the module-level DEFAULT_PROFILE constant.
...
def default_songs():
    """Return a default list of songs shown when the app first loads."""
    return [
        ...
        {"title": "Sweet Child O' Mine", "artist": "Guns N' Roses", "genre": "rock", "energy": 8, "tags": ["guitar", "80s"]},
        {"title": "Bad Guy", "artist": "Billie Eilish", "genre": "pop", "energy": 6, "tags": ["bass", "dark"]},
        ...
        {"title": "Clair de Lune", "artist": "Claude Debussy", "genre": "ambient", "energy": 2, "tags": ["piano", "classical"]},
        {"title": "Hotel California", "artist": "Eagles", "genre": "rock", "energy": 6, "tags": ["classic", "guitar"]},
...
# Sidebar controls
# -----------------------------
def profile_sidebar() -> None:
    """Render and update the user profile."""
    st.sidebar.header("Mood profile")

    profile = st.session_state.profile

    profile["name"] = st.sidebar.text_input("Profile name", value=str(profile.get("name", "")))
    ...
    current = str(profile.get("favorite_genre", "rock") or "rock").lower().strip()
    index = genre_options.index(current) if current in genre_options else 0

    profile["favorite_genre"] = st.sidebar.selectbox("Favorite genre", options=genre_options, index=index)
...
def add_song_sidebar() -> None:
    """Render the Add Song controls in the sidebar."""
    st.sidebar.header("Add a song")

    title = st.sidebar.text_input("Title")
    artist = st.sidebar.text_input("Artist")
    genre = st.sidebar.selectbox("Genre", options=["rock", "lofi", "pop", "jazz", "electronic", "ambient", "other"])
    ...
    if st.sidebar.button("Add to playlist"):
        # I normalize inputs to avoid duplicates caused by casing/whitespace.
        raw_tags = [t.strip() for t in tags_text.split(",")]
        tags = [t for t in raw_tags if t]

        song: Song = {"title": title, "artist": artist, "genre": genre, "energy": energy, "tags": tags}

        if not title or not artist:
            st.sidebar.warning("Please provide at least a title and an artist.")
            return

        normalized = normalize_song(song)

        # Basic duplicate prevention: if the same title+artist+energy exists, don't add it twice.
        existing = [normalize_song(s) for s in st.session_state.songs]
        exists = any(
            (s["title"].strip().lower() == normalized["title"].strip().lower())
            and (s["artist"] == normalized["artist"])
            and (int(s["energy"]) == int(normalized["energy"]))
            for s in existing
        )
        if exists:
            st.sidebar.info("That song already exists in your list (after normalization).")
            return

        st.session_state.songs = st.session_state.songs + [normalized]
        st.sidebar.success("Added!")


def clear_controls() -> None:
    """Render controls for resetting data."""
...
def playlist_tabs(playlists):
    ...
    include_mixed = bool(st.session_state.profile.get("include_mixed", True))
...
def render_playlist(label, songs):
    ...
    # The spec says search should be partial + case-insensitive.
...
def lucky_section(playlists):
    ...
    mode = st.selectbox("Pick from", options=["any", "hype", "chill"], index=0)
    ...
    if st.button("Feeling lucky"):
        ...
        st.success(f"Lucky song: {pick['title']} by {pick['artist']} (mood {pick.get('mood', '?')})")

        # Keep a simple history so students can see how state changes in Streamlit.
        st.session_state.history = st.session_state.history + [pick]
...
def main():
    ...
    st.write(
        ...
        "The UI is friendly, but the logic was intentionally chaotic for debugging practice."
    )
    ...
    # Build playlists from the current songs and profile.
    base_playlists = build_playlists(songs, profile)

    # The starter app merged playlists for no reason; I keep the call but fix merge_playlists
    # so it can't accidentally mutate lists in-place.
